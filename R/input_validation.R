...
.scholid_match_type <- function(type) {
    type_chr <- .scholid_as_scalar_character(type, arg = "type")
    if (is.na(type_chr)) {
        stop("`type` must be a non-empty string.", call. = FALSE)
    }

    choices <- scholid_types()
    out <- match.arg(type_chr, choices = choices, several.ok = FALSE)

    if (!identical(type_chr, out)) {
        stop("`type` must match exactly; abbreviations are not allowed.",
             call. = FALSE)
    }
